...
**Kullanıcıda bekleyenler:** SQL Editor'de `20260719010000_add_families.sql`; isteğe bağlı TMDB "API Read Access Token".

**Aile özelliği (kararlaştırılmış tasarım):** çoklu grup destekli aileler; kod tabanlı davet (e-posta eşleşmeli, 7 gün, RPC accept_family_invite); profiles (görünen ad) üyelik önkoşulu; module_shares (modül × Kapalı/Özet/Tam — Özet yalnız bütçede). Dilim 1 tamam (kur/davet/katıl/üye yönetimi/paylaşım seçimi + Ailem sayfası + Özet kartı). **Dilim 2 (sırada):** paylaşılan modüllerin üyelerce görüntülenmesi — veri tablolarına aile-görünürlük SELECT policy'leri (full) + bütçe özeti için security-definer RPC (summary) + Ailem'de üye modül kartları. **Dilim 3:** ortak harcama alanı (transactions.family_id + Tricount tarzı denge), Faz 3 gezi grupları aynı altyapıda.
