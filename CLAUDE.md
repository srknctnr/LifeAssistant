...
**Kullanıcıda bekleyenler:** SQL Editor'de `20260720010000_add_share_visibility.sql`; isteğe bağlı TMDB "API Read Access Token".

**Aile özelliği (kararlaştırılmış tasarım):** çoklu grup destekli aileler; kod tabanlı davet (e-posta eşleşmeli, 7 gün, RPC accept_family_invite); profiles (görünen ad) üyelik önkoşulu; module_shares (modül × Kapalı/Özet/Tam — Özet yalnız bütçede). Dilim 1 tamam (kur/davet/katıl/üye yönetimi/paylaşım seçimi). Dilim 2 tamam: full paylaşımlar veri tablolarında aile SELECT policy'leriyle, bütçe özeti family_budget_summary RPC'siyle görünür; Ailem'de üye paylaşım rozetleri read-only görüntüleme panelleri açıyor; tüm "kendi verim" sorguları currentUserId ile sahibine sabitlendi (karışma olmaz). **Dilim 3 (sırada):** ortak harcama alanı (transactions.family_id + Tricount tarzı denge), Faz 3 gezi grupları aynı altyapıda.
