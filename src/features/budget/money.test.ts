...
describe('previousMonthCarry', () => {
  const today = new Date(2026, 1, 10) // 10 Şubat 2026 — geçen ay Ocak
  const incomes = [
    { amount: 60000, income_date: null, created_at: '2025-12-01T00:00:00Z' },
  ]
  const expenses = [
    {
      amount: 29000,
  ...
  ] // Ocak'ta harcanabilir: 31.000₺

  it('carries last month overspend forward as a debt', () => {
    const carry = previousMonthCarry({
      incomes,
      expenses,
      transactions: [{ amount: 36000, category: null, spent_on: '2026-01-20' }],
      today,
    })
    expect(carry).toBe(-5000)
  })

  // A month with nothing logged is indistinguishable from a month of perfect
  // thrift. Carrying a surplus forward would invent money out of a gap in the
  // record, so only debt travels.
  it('never carries a surplus forward, however large', () => {
    expect(
      previousMonthCarry({ incomes, expenses, transactions: [], today }),
    ).toBe(0)
    expect(
      previousMonthCarry({
        incomes,
        expenses,
        transactions: [
          { amount: 1000, category: null, spent_on: '2026-01-05' },
        ],
        today,
      }),
    ).toBe(0)
  ...
  it('ignores this month and the month before last', () => {
    const carry = previousMonthCarry({
      incomes,
      expenses,
      transactions: [
        { amount: 90000, category: null, spent_on: '2026-02-02' }, // bu ay
        { amount: 90000, category: null, spent_on: '2025-12-15' }, // iki ay önce
      ],
      today,
    })
    expect(carry).toBe(0)
  ...
  it('owes nothing for a month the user was not there for', () => {
    const carry = previousMonthCarry({
      incomes: [
        {
          amount: 60000,
  ...
  it('crosses a year boundary', () => {
    const carry = previousMonthCarry({
      incomes,
      expenses,
      transactions: [{ amount: 40000, category: null, spent_on: '2025-12-20' }],
      today: new Date(2026, 0, 10), // 10 Ocak → geçen ay Aralık 2025
    })
    expect(carry).toBe(-9000)
  })
})

describe('paceReport — devreden borç', () => {
  const today = new Date(2026, 6, 10)
  const base = { monthlyIncome: 60000, plannedExpense: 29000, today }

  it('takes the debt off the top without rewriting the plan', () => {
    const report = paceReport({ ...base, transactions: [], carry: -5000 })
    expect(report.planSpendable).toBe(31000)
    expect(report.carry).toBe(-5000)
    expect(report.spendable).toBe(26000)
    expect(report.remaining).toBe(26000)
  })

  it('leaves everything untouched when nothing is owed', () => {
    const withZero = paceReport({ ...base, transactions: [], carry: 0 })
    const without = paceReport({ ...base, transactions: [] })
    expect(withZero).toEqual(without)
    expect(without.spendable).toBe(without.planSpendable)
  })

  // The two invariants the previous slices pinned must survive a debt.
  it('still drops the headline by exactly what you log', () => {
    const before = paceReport({ ...base, transactions: [], carry: -5000 })
    const after = paceReport({
      ...base,
      carry: -5000,
      transactions: [{ amount: 200, spent_on: '2026-07-10' }],
    })
    expect(toMinor(before.todayLeft) - toMinor(after.todayLeft)).toBe(20000)
    expect(after.todayBudget).toBe(before.todayBudget)
  })

  it("still promises exactly tomorrow's budget", () => {
    const transactions = [{ amount: 9000, spent_on: '2026-07-03' }]
    const tonight = paceReport({ ...base, transactions, carry: -5000 })
    const tomorrow = paceReport({
      ...base,
      transactions,
      carry: -5000,
